...
from bot import update_log, check_weather, get_money, fact, history, date, LOG_HISTORY, get_data_with_url
...
class TestsFact(unittest.TestCase):
    ...
    def test_get_two_most_upvoted(self):
        with patch('bot.get_data_with_url') as mock_data:
            mock_data.return_value = {'all': [{'upvotes': 1, 'text': 'text message'}, {'upvotes': 1, 'text': 'text message_2'}]}
            most_upvoted = fact()
