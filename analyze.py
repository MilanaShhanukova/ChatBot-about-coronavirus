...
class Statistics:
    ...
    @staticmethod
    def deaths(shift: int, location: str):
        now = datetime.datetime.today()
        r = requests.get(
            f"https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv")
        now = now - datetime.timedelta(days=1)
        date = now - datetime.timedelta(days=shift)
        dates = []  # массив с датами до сегодня
        data_set = []  # массив с координатами
        with open("current_deaths_info.csv", 'w', encoding='utf-8') as now_data:
            now_data.writelines(r.text)
        with open("current_deaths_info.csv", encoding='utf-8') as now_data:
            reader = csv.DictReader(now_data)
            while date < now:
                date_1 = date.strftime(
                    "%m/%d/%Y")  # для поиска в таблице нужен другой формат даты, создаем его в этой переменной
    ...
    @staticmethod
    def grafik_deaths_draw(shift: int, location: str):
        dates, data_set = Statistics.deaths(shift, location)
        y = data_set
        # соответствующие значения оси Y
        x = dates
        fig, ax = plt.subplots()
